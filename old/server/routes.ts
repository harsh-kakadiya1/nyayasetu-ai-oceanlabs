...
import { insertDocumentSchema, insertAnalysisSchema, insertChatMessageSchema } from "../shared/schema.js";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post(["/api/auth/register", "/api/auth-register"], async (req, res) => {
    try {
      ...
      console.log(`[REGISTER] New registration attempt for: ${username}`);
      ...
      console.log(`[REGISTER] Checking if user exists in database...`);
      const existingUser = await storage.getUserByUsername(username);
      if (existingUser) {
        console.log(`[REGISTER] ✗ User already exists: ${username} (ID: ${existingUser.id})`);
      ...
      console.error("[REGISTER] Error type:", (error as any).constructor.name);
      console.error("[REGISTER] Error message:", (error as any).message || error);
      console.error("[REGISTER] Error code:", (error as any).code);
      console.error("[REGISTER] ────────────────────────────────────────────");
      res.status(500).json({ error: "Failed to register user: " + (error as any).message });
    }
  });
  ...
  app.post(["/api/auth/login", "/api/auth-login"], (req, res, next) => {
    ...
    passport.authenticate("local", (err: any, user: any, info: any) => {
      ...
      if (!user) {
        console.log(`[AUTH] Authentication failed for user: ${username}. Info:`, info);
        return res.status(401).json({ error: info?.message || "Authentication failed" });
      }
      
      req.login(user, (loginErr: any) => {
        if (loginErr) {
          console.error(`[AUTH] Session establishment error: ${loginErr.message}`);
          return res.status(500).json({ error: "Failed to establish session" });
        }
        
        console.log(`[AUTH] Login successful for user: ${username}, session established`);
        res.json({ id: user.id, username: user.username });
      });
    })(req, res, next);
  });
  ...
  app.get(["/api/analysis/:id/messages", "/api/analysis-messages"], requireAuth, async (req, res) => {
    try {
      ...
      console.log(`[MESSAGES] User authenticated: ${req.user?.id}`);
