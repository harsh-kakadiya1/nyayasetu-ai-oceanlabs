...
const loadingSteps = [
  {
    key: "parsing",
    icon: FileText,
    duration: 1500, // 1.5 seconds per step, total 6 seconds
  },
  {
    key: "analyzing", 
    icon: Zap,
...
export default function LoadingAnalysis({ className = "" }: LoadingAnalysisProps) {
  ...
  const [showSkeleton, setShowSkeleton] = useState(false); // Don't show skeleton initially
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-card rounded-lg border border-border p-8 sm:p-12 text-center">
        ...
        {/* Current Step */}
        <div className="mb-6">
          <p className="text-2xl sm:text-3xl font-semibold text-primary mb-2">
      ...
      {/* Skeleton Loading - Shows after 1 second of loading */}
      {showSkeleton && (
