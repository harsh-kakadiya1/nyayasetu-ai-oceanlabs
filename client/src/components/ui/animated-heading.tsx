...
function AnimatedHeading() {
  ...
  const translatedLines = t("landing.hero.lines", { returnObjects: true }) as unknown;

  const titles = useMemo(
    () => {
      if (Array.isArray(translatedLines)) {
        const lines = translatedLines.filter(
      ...
      return [t("landing.hero.title"), t("landing.hero.highlight")].filter(
        (line) => line.trim().length > 0
      );
    },
    [i18n.resolvedLanguage, t, translatedLines]
  );
  ...
  return (
    <div>
      <h1 className="font-display mx-auto max-w-4xl text-4xl font-semibold leading-[1.02] tracking-tight text-[#1b3338] sm:text-6xl sm:leading-[0.98] lg:text-7xl">
        ...
        <span className="relative mt-2 block min-h-[2.8em] overflow-hidden text-[#a24e2f] sm:min-h-[2.4em]">
